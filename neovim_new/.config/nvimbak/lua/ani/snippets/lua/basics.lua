...
return {
	...
	s(
		{
			trig = "req",
			name = "local require",
			dscr = "auto sets variable name according to path",
			-- snippetType = "autosnippet",
		},
		fmta(
			[[
	...
	s(
		{
			trig = "map",
			name = "keymap block (choice)",
			dscr = "Insert keymap block or add map entry via choice node",
		},

		c(1, {
			-- Choice 1: Append maps entry
			sn(
				nil,
				fmta(
					[[
					...
					{
						f(function(value)
							local string = value[1][1]
							local description = string:gsub("%[(.-)%]", "%1")
							return "This is a Snippet that " .. description
						end, { 3 }), -- Description
						i(1), -- key
						i(2, "n"), -- mode
						i(3), -- desc
				...
				fmta(
						...
						]],
					{
						...
						f(function(value)
							local string = value[1][1]
							local description = string:gsub("%[(.-)%]", "%1")
							return "This is a snippet to " .. description
						end, { 7 }), -- Description
						i(5), -- key
						i(6), -- mode
						i(7), -- desc
